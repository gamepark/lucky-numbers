...
import {boardLeft, boardTop, cloverSize, discardMarginLeft, discardMarginTop} from '../styles'
...
export default function Board({garden, idGarden, isMine, isSetupPhase, cloversDiscarded, playerPosition, itemDrag, isBrunoVariant, selectedClover, cloverInHand, ...props}: Props) {
  ...
  return (
    <div css={[style(itemDrag, playerPosition === 0), isMine && itemDrag !== null && !isGoodCloverGoodTime({color:itemDrag.color,number:itemDrag.number}, actionsNumber) && tutorial !== undefined && wrongCloverTutoStyle(t(actionsNumber === 10 ? "tuto.discard" : "tuto.wrong.clover"))]} {...props}>
      {garden.map((line, row) =>
...
const style = (itemDrag:Clover, isMine:boolean|undefined) => css`
  position: absolute;
